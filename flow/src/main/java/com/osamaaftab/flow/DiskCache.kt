...
import com.osamaaftab.cache.DiskLruCache
import java.io.*
import java.math.BigInteger
import java.security.MessageDigest
import java.security.NoSuchAlgorithmException

class DiskCache private constructor(val context: Context, size: Int) :
    ImageCache {
    private var cache: DiskLruCache =
        DiskLruCache.open(context.cacheDir, 1, 1, (size.toLong() * 1024 * 1024))
    ...
    companion object {
        ...
        @Synchronized
        fun getInstance(context: Context, size: Int): DiskCache {
            return INSTANCE?.let { return INSTANCE }
                ?: run {
                    return DiskCache(context, size)
                }
        }
    }
}